...
from dataprocess import dataTrainAugmentation, dataHoldOutAugmentation, get_mounted_data, \
    transform_image_dataframe_to_matrix
...
class Gater(object):
    ...
    def train_model(self, X, y, x_val, y_val, i):
        model = nn_models()
        model.ip_shape = (612, 250, 250, 3) #X.shape
        model = model.LeNet5()
    ...
    def main(self, x_train, y_train, x_test, y_test, x_val, y_val):
        ...
        model_prime.ip_shape = (204, 250, 250, 3) #x_train.shape
...
def main():
    ...
    train_data_generator = dataTrainAugmentation().flow_from_directory(
        directory=os.path.join(os.getcwd(), 'training_data_Inception2_5_1'),
        target_size=(250, 250),
    ...
    valid_data_generator = dataTrainAugmentation().flow_from_directory(
        directory=os.path.join(os.getcwd(), 'validation_data_Inception2_5_1'),
        target_size=(250, 250),
        batch_size=BATCH_SIZE,
        shuffle=False
    )
    ...
    gater = Gater()
    gater.experts = 3
    gater.train_dim = (612, 250, 250, 3)
    gater.test_dim = (204, 250, 250, 3)
    print("Experts {}".format(gater.experts))
    print('{}, {}, {}, {}'.format("Training Error", "Val Error", "Test Error", "Time"))
    gater.main(train_data_generator, train_data_generator.classes, valid_data_generator, valid_data_generator.classes, valid_data_generator, valid_data_generator.classes)
